"""
Pizza detection model using Roboflow inference SDK.

This model uses the Roboflow inference SDK to detect pizzas in images.
...
class PizzaDetectModel(YoloPythonModel):
    """MLflow PyFunc model for pizza detection using Roboflow inference.

    This model uses the Roboflow inference SDK to detect pizzas.
    Requires ROBOFLOW_API_KEY environment variable to be set.
    """

    ROBOFLOW_PROJECT = "pizza-r2sci"
    ROBOFLOW_VERSION = 23
    ...
    def load_context(self, context):
        ...
        self.model = get_model(
            model_id=PizzaDetectModel.MODEL_ID,
    ...
    def predict(self, context, model_input: pd.DataFrame) -> pd.DataFrame:
        """Run pizza detection on batch of images.
        ...
        Returns:
            DataFrame with 'detections' column containing pizza detection results.
        """
        images = []
        row_map = []
...
mlflow.models.set_model(PizzaDetectModel())
